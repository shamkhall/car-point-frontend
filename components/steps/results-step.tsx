...
export function ResultsStep({
  ...
  const PriceIcon =
    price.deviation < 0 ? TrendingDown : price.deviation > 0 ? TrendingUp : Minus;

  const breakdownItems = scoreBreakdownConfig.map((item) => ({
    category: item.label,
    score: scoreBreakdown[item.key as keyof typeof scoreBreakdown],
    maxScore: item.max,
  }));
    ...
    <div className="flex-1 flex flex-col px-6 py-8 overflow-y-auto">
      <div className="max-w-lg mx-auto w-full space-y-8">
        ...
        <div className="flex flex-col items-center gap-4">
          <div className="relative w-48 h-48 flex items-center justify-center">
            <svg className="absolute inset-0 w-full h-full -rotate-90">
              ...
              <circle
                ...
                strokeLinecap="round"
                strokeDasharray={553}
                strokeDashoffset={553 - (553 * qualityScore) / 100}
                className={cn(
          ...
          <div
            className={cn(
              "px-4 py-2 rounded-full font-medium uppercase text-sm",
              qualityFullColors[qualityStatus]
            )}
          >
        ...
        <div className="bg-card border border-border rounded-2xl p-6 space-y-4">
          <div className="flex items-center justify-between">
            ...
            <div
              className={cn(
                ...
                priceTextColors[price.priceStatus]
              )}
            >
          ...
          <div className="text-center py-2">
            ...
            >
              {price.average != null
                ? `Bazardan ${Math.abs(price.deviation).toFixed(1)}% ${
                    price.deviation <= 0 ? "aşağı" : "yuxarı"
                  }`
            ...
            <div className="space-y-2">
              ...
              <div className="relative h-3 bg-secondary rounded-full">
                <div
                  ...
                  style={{
                    left: `${Math.min(
                      Math.max((price.listed / (price.average * 1.5)) * 100, 5),
                      95
                    )}%`,
                  }}
                />
                <div
                  ...
                  style={{
                    left: `${Math.min(
                      Math.max(
                        (price.average / (price.average * 1.5)) * 100,
                        5
                      ),
                      95
                    )}%`,
